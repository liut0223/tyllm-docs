...
- ``TyQuant``：云天励飞大模型量化工具，用户可通过本工具或其它开源量化工具对大模型完成量化
- ``TyLLM``：云天励飞推出的大模型工具链，基于``TyTVM``工具链针对大模型增量开发，主要基于``PyTorch``和``vLLM``做专属优化，帮助用户将大模型编译为``Edge10``系列芯片可执行模型
- ``TyTVM``：云天模型转换、量化、仿真、编译工具链，负责将模型编译为芯片执行的模型
...
### 环境准备
``TyLLM``使用``Docker``容器进行工具链集成，需准备以下环境：
- **硬件要求**：Nvidia GPU
- **软件依赖**：
...
sudo docker -v
# 预期输出：Docker version 20.10.21, build 20.10.21-0ubuntu1~20.04.2
```
...
#### 配置Docker镜像仓库
需要将``113.100.143.90:8091``加入Docker不安全注册表：
```json
{     
...
#### 安装TyQuant量化工具
1. 拉取镜像（将`${version}`替换为实际版本号，如`v1.0.2`）：
```shell
...
```
> 说明：`${your_data_dir}`表示宿主机中用户数据目录

#### 安装TyLLM工具链
1. 拉取镜像（将`${version}`替换为实际版本号，如`v1.1.7`）：
```shell
sudo docker login 113.100.143.90:8091 -u custom -p DE@sz_intellif_2021
...
## 模型量化
建议使用带GPU的主机加快量化速度，目前仅支持AWQ量化方式。

### 量化示例1-LLM模型（以Qwen3-1.7B为例）
```python
import torch
...
### 量化示例2-VLM模型（以Qwen2.5-vl-7B为例）
> 注：仅支持language model的AWQ量化，visual model的AWQ量化暂不支持
...
2. 评估配置（eval_tmp.yaml）：
```yaml
eval_backend: VLMEvalKit
eval_config:
  model: 
    - type: Qwen3-1.7b-VL  # 按照启动脚本的--model填写
      name: CustomAPIModel 
...
#### 参数说明
- **model_path(str)**：huggingface模型和配置文件的路径
- **max_kv_cache_size(int, optional)**：kv缓存的最大容量，默认为4096
- **seq_len_list(list of int, optional)**：用于构建和编译模型的序列长度列表，默认为[1, 8]
- **dev_count(int, optional)**：用于运行已编译模型的设备数量（如 NNP 设备），默认为1
- **artifacts_path(str, optional)**：保存模型编译产物的目录路径，默认使用model_path
- **embedding_as_input(bool, optional)**：如果为True，将提取嵌入层并单独保存为NumPy数组，默认为False
...
### 视觉语言大模型（基于vLLM的Qwen2.5-VL-7B-Instruct示例）
```python
import os
import logging
import datetime
...
#### 编译说明
- 文件处理代码将部分生成产物改名，以匹配云天大模型SDK的文件名要求
- AOT_DIR建议配置为`Qwen3-VL-4B...`格式（中间用-隔开），以匹配板上运行的云天大模型SDK目录名要求
- VM_die_remap：配置视觉部分Die分配顺序，list长度必须为16
- LM_die_remap：配置语言部分Die分配顺序，list长度必须为16
- IMAGE_ORG_PATH可配置本地任意图片，用于生成图像输入
- GPU环境下编译过程有一定加速；无GPU时可尝试将`COMPILE_THREAD`配置为"2"以多线程加速

#### 注意事项
- 编译时若报缺少`config.json`、`preprocessor_config.json`、`vocab.json`等文件，可能是量化过程文件未拷贝完整，需从浮点模型目录手动拷贝相应文件到量化模型目录
- 若因错误需要重启编译任务，建议删除已生成的文件后重新运行脚本
...
**问题描述**：编译过程中出现段错误，可能是pyarrow包版本问题  
**解决方法**：将pyarrow包降级至16.0.0  
```shell
pip install pyarrow==16.0.0
```

### 2. 编译阶段出现 RuntimeError: Expected all tensors to be on the same device
**问题描述**：GPU环境须限制编译线程环境变量  
**解决方法**：设置环境变量  
```python
os.environ["COMPILE_THREAD"] = "1"
```
...
**问题描述**：检测到GPU设备时，编译脚本对全局设置有顺序要求  
**解决方法**：保持文档中编译脚本各操作先后顺序不做改动
```
```
